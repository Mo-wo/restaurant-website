...
export const Pagination: React.FC<PaginationProps> = ({ totalPages, currentPage, onPageChange }) => {
  ...
  return (
    <div className={styles.paginationWrapper}>
      {currentPage > 4 && (
        <button onClick={() => handlePageChange(currentPage - 4)} className={styles.textButton}>...</button>
      )}
      ...
      {currentPage <= totalPages - 4 && (
        <button onClick={() => handlePageChange(currentPage + 4)} className={styles.textButton}><IoEllipsisHorizontalSharp />
</button>
      )}
    </div>
  );
